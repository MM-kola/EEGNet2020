...
def format_data(Signal, Flashing, StimulusCode, StimulusType, Target):
    ...
    P300_dataset = np.zeros([85, 2, 15, T, 64])
    non_P300_dataset = np.zeros([85, 10, 15, T, 64])
    trainset = responses
    print('trainset', trainset.shape)
    #####################################################################################################################
    ...
    for char_epoch in range(0, trainset.shape[0]):
        ...
        for i in range(0, trainset.shape[2]):
            l = 0
            for index in range(0, 12):
                if index == ind_1 - 1 or index == ind_2 - 1:
                    P300_dataset[char_epoch, 0, i, :, :] = trainset[char_epoch, ind_1 - 1, i, :, :]
                    P300_dataset[char_epoch, 1, i, :, :] = trainset[char_epoch, ind_2 - 1, i, :, :]
                else:
                    # print ('here')
                    # print (index)
                    non_P300_dataset[char_epoch, l, i, :, :] = trainset[char_epoch, index, i, :, :]
...
def Fdataset(P300_train, P300_hold, non_P300_train, P300_train_label, P300_hold_label, non_P300_train_label,
    ...
    train_label[P300_train_label.shape[0]:P300_train_label.shape[0] + non_P300_train_label.shape[0], :] = non_P300_train_label[:, :]
    ...
    hold_label[P300_hold_label.shape[0]:P300_hold_label.shape[0] + non_P300_hold_label.shape[0], :] = non_P300_hold_label[:, :]
...
def makeset():
    ...
    for i in range(0, Signal_A_240.shape[0]):
        if subject == '0':
            ...
            #     'int8')
            # StimulusCode_A[i, :] = pooling(StimulusCode_A_240[i, :], 2)
            ...
            StimulusType_B[i, :] = abs(np.round(scipy.signal.resample(StimulusType_B_240[i, :], int(samps))))
    StimulusCode_A = pooling(StimulusCode_A_240, 2)
    #      TO DO :
    ...
    targets_ = np.zeros([train_char_size * 15 * 12, 12])
    targets_[:, 0:2] = 1
    targets_[:, 2:12] = 0
    ## Create training and validation subset
    P300_train_, P300_hold_, P300_train_label_, P300_hold_label_ = create_subset(
        np.reshape(P300_dataset_, [train_char_size * 2 * 15, 72, 64]),
        np.reshape(targets_[:, 0:2], [train_char_size * 2 * 15 * 12, 1]))
    non_P300_train_, non_P300_hold_, non_P300_train_label_, non_P300_hold_label_ = create_subset(
        np.reshape(non_P300_dataset_, [train_char_size * 10 * 15, 72, 64]),
        np.reshape(targets_[:, 2:12], [train_char_size * 10 * 15 * 12, 1]))
...
def main():
    ...
    scipy.io.savemat('G:\EEGNet\data\EEGDataset\SingleTaril\TrainDatas/Normal\TrainDataset.mat', dataset_train)
    scipy.io.savemat('G:\EEGNet\data\EEGDataset\SingleTaril\TrainDatas/Normal\HoldDataset.mat', dataset_hold)
