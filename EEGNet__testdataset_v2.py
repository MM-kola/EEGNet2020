...
def format_test_data(Signal, Flashing, StimulusCode, Target):
    ...
    test_P300_dataset = np.zeros([test_char_size, 2, 15, T, 64])
    test_non_P300_dataset = np.zeros([test_char_size, 10, 15, T, 64])
    for char_epoch in range(0, testset.shape[0]):
        ...
        # print (ind_1,ind_2)
        for i in range(0, testset.shape[2]):
            l = 0
            for index in range(0, 12):
                if index == ind_1 - 1 or index == ind_2 - 1:
                    test_P300_dataset[char_epoch, 0, i, :, :] = testset[char_epoch, ind_1 - 1, i, :, :]
                    test_P300_dataset[char_epoch, 1, i, :, :] = testset[char_epoch, ind_2 - 1, i, :, :]

                else:
                    # print ('here')
                    # print (index)
                    test_non_P300_dataset[char_epoch, l, i, :, :] = testset[char_epoch, index, i, :, :]
...
P300_test_A = np.reshape(test_P300_dataset_A, [100 * 2 * 15, 72, 64])
...
non_P300_test_A = np.reshape(test_non_P300_dataset_A, [100 * 10 * 15, 72, 64])
...
P300_test_B = np.reshape(test_P300_dataset_B, [100 * 2 * 15, 72, 64])
...
non_P300_test_B = np.reshape(test_non_P300_dataset_B, [100 * 10 * 15, 72, 64])
...
dataset_A_test1['labels'] = targets_A_test
dataset_B_test1 = {};
dataset_B_test1['data'] = dataset_B_test
dataset_B_test1['labels'] = targets_B_test



scipy.io.savemat('G:\EEGNet\data\EEGDataset\SingleTaril\TestDatas/Normal/test_set_A.mat', dataset_A_test1)
scipy.io.savemat('G:\EEGNet\data\EEGDataset\SingleTaril\TestDatas/Normal/test_set_B.mat', dataset_B_test1)
# scipy.io.savemat('G:\EEGNet\data\EEGDataset\SingleTaril\TestDatas\Circle/test_set_A.mat', dataset_A_test1)
# scipy.io.savemat('G:\EEGNet\data\EEGDataset\SingleTaril\TestDatas\Circle/test_set_B.mat', dataset_B_test1)
# %%
